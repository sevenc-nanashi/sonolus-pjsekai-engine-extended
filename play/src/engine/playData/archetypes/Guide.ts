import { Color } from "../../../../../shared/src/engine/data/Color.js";
import { FadeType } from "../../../../../shared/src/engine/data/FadeType.js";
import { options } from "../../configuration/options.js";
import { skin } from "../skin.js";
import { layer } from "./layer.js";
import { Note } from "./notes/Note.js";
import { type EaseType, ease } from "./slideConnectors/EaseType.js";
import {
  getScheduleSFXTime,
  getZwithLayer,
...
export class Guide extends Archetype {
  ...
  renderConnector() {
    if (
      options.hidden > 0 &&
      timeToScaledTime(time.now, this.data.headTimeScaleGroup) >
        this.visualTime.hidden
    )
      return;
    ...
    for (let i = 0; i < options.guideQuality; i++) {
      const scaledTime = {
        min: Math.lerp(
          visibleTime.min,
          visibleTime.max,
          i / options.guideQuality,
        ),
        max: Math.lerp(
          visibleTime.min,
          visibleTime.max,
          (i + 1) / options.guideQuality,
        ),
      };
