import { SlideStartType } from "~shared/engine/data/SlideStartType.js";
...
import type { SlideStartNote } from "../notes/flatNotes/slideStartNotes/SlideStartNote.js";
...
import { type EaseType, ease } from "./EaseType.js";
...
export abstract class SlideConnector extends Archetype {
  ...
  preprocess() {
    ...
    this.head.scaledTime = timeToScaledTime(
      this.head.time,
      this.headData.timeScaleGroup,
    );
    ...
    this.start.scaledTime = timeToScaledTime(
      this.start.time,
      this.startData.timeScaleGroup,
    );
    this.end.time = bpmChanges.at(this.endData.beat).time;
    this.end.scaledTime = timeToScaledTime(
      this.end.time,
      this.endData.timeScaleGroup,
    );
    ...
    this.tail.scaledTime = timeToScaledTime(
      this.tail.time,
      this.tailData.timeScaleGroup,
    );
    ...
    if (options.hidden > 0)
      this.visualTime.hidden =
        this.tail.scaledTime - Note.duration * options.hidden;
  ...
  updateParallel() {
    ...
    if (
      ...
      Math.abs(this.getL(s) - this.getR(s)) < 0.25
    )
      return;
  ...
  get shouldScheduleSFX() {
    return (
      ...
      (this.useFallbackClip
        ? this.clips.fallback.exists
        : this.clips.hold.exists) &&
      options.autoSFX
    );
  }
  ...
  get useFallbackConnectorSprites() {
    return (
      !this.sprites.connector.normal.exists ||
      !this.sprites.connector.active.exists
    );
  }
  ...
  spawnCircularEffect() {
    this.effectInstanceIds.circular = this.effects.circular.spawn(
      new Quad(),
      1,
      true,
    );
  ...
  spawnLinearEffect() {
    this.effectInstanceIds.linear = this.effects.linear.spawn(
      new Quad(),
      1,
      true,
    );
    ...
    for (let i = 0; i < options.slideQuality; i++) {
      const scaledTime = {
        min: Math.lerp(
          visibleTime.min,
          visibleTime.max,
          i / options.slideQuality,
        ),
        max: Math.lerp(
          visibleTime.min,
          visibleTime.max,
          (i + 1) / options.slideQuality,
        ),
      };
  ...
  renderSlide() {
    ...
    if (this.data.startType === SlideStartType.Trace) {
      ...
      if (this.useFallbackSlideSprites) {
        this.sprites.slide.fallback.draw(
          perspectiveLayout({ l, r, b, t }),
          this.slide.z,
          1,
        );
