...
export const getClaimedStart = (index: number) =>
  claimStartManager.getClaimedTouchIndex(index);
...
export const getClaimedEnd = (index: number) =>
  claimEndManager.getClaimedTouchIndex(index);

const disallowedTraceStarts = levelMemory(Collection(16, TouchId));

export const canTraceStart = (touch: Touch) =>
  !disallowedTraceStarts.has(touch.id);

export const disallowTraceStart = (touch: Touch) =>
  disallowedTraceStarts.add(touch.id);
