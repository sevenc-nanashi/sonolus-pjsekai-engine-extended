...
export class SimLine extends Archetype {
  ...
  preprocess() {
    ...
    this.leftVisualTime.max = timeToScaledTime(
      this.leftTargetTime,
      this.leftTimeScaleGroup,
    );
    this.leftVisualTime.min = this.leftVisualTime.max - Note.duration;

    this.rightVisualTime.max = timeToScaledTime(
      this.rightTargetTime,
      this.rightTimeScaleGroup,
    );
  ...
  initialize() {
    ...
    if (options.hidden > 0)
      this.leftVisualTime.hidden =
        this.leftVisualTime.max - Note.duration * options.hidden;
